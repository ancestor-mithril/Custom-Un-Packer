...
class CustomUnPackerTester(unittest.TestCase):
    ...
    def test_list_content(self):
        ...
        try:
            run_create_archive(target_archive, "./test/test_folder")
            list_archive_content = [x.replace("\\", "/") for x, y in get_archive_content(target_archive)[1]]
        except Exception as e:
            error_print(e)
            no_error = False
        shutil.rmtree("./test")
    ...
    def test_unpack(self):
        ...
        try:
            ...
            unpacked_files = [x.replace("\\", "/") for x in
                              get_local_path_of_files_from_directory("./test/archived_folder")]
        except Exception as e:
            error_print(e)
            no_error = False
        shutil.rmtree("./test")
